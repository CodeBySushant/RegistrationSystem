...
const emptyTransferRow1 = () => ({ date: "", name: "", recommender: "", approver: "" });
const emptyTransferRow2 = () => ({ from_name: "", to_name: "", recommender: "", approver: "" });
...
const initialState = {
  ...
  purpose: "",
  floor_details: initialFloorDetails,
  height: "",
  setback_distance: "",
  setback_value: "",
  electric_distance: "",
  ...
  bodartha_name: "",
  bodartha_ward: "",
  transfers1: [emptyTransferRow1()],
  transfers2: [emptyTransferRow2()],
  applicantName: "",
  applicantAddress: "",
...
export default function BhawanNirmanSampanna() {
  ...
  // Unified handleChange for ApplicantDetailsNp compatibility
  ...
  // Transfers table 1
  const addTransfer1 = () =>
    setForm((p) => ({ ...p, transfers1: [...p.transfers1, emptyTransferRow1()] }));
  ...
  const removeTransfer1 = (idx) =>
    setForm((p) => ({ ...p, transfers1: p.transfers1.filter((_, i) => i !== idx) }));

  // Transfers table 2
  const addTransfer2 = () =>
    setForm((p) => ({ ...p, transfers2: [...p.transfers2, emptyTransferRow2()] }));
  ...
  const removeTransfer2 = (idx) =>
    setForm((p) => ({ ...p, transfers2: p.transfers2.filter((_, i) => i !== idx) }));
  ...
  const buildPayload = () => ({
    ...
    bodartha_name: form.bodartha_name,
    bodartha_ward: form.bodartha_ward,
  ...
  const handlePrint = async () => {
    if (!form.ward_no || !form.owner_name) {
      alert("कृपया अनिवार्य फिल्डहरू भर्नुहोस्।");
      return;
    }
    setLoading(true);
    try {
      const res = await axios.post("/api/forms/bhawan-nirman-sampanna", buildPayload());
      if (res.status === 201) {
      ...
      <div className="form-body">
        ...
        <div className="details-list">
          ...
          <div className="detail-item">
            ...
            <label> मिति : </label>
            <input value={form.map_pass_date} onChange={(e) => setField("map_pass_date", e.target.value)} type="text" className="dotted-input medium-input" />
          </div>
          ...
          <div className="floor-details-grid pl-20">
            {form.floor_details.map((r, i) => (
              <div key={r.key} className="grid-row">
                <label>{r.label}</label>
                <input
                  value={r.value}
        ...
        <div className="bodartha-section mt-30">
          ...
          <p>
            श्री
            <input
              value={form.bodartha_name}
              onChange={(e) => setField("bodartha_name", e.target.value)}
              type="text"
              className="dotted-input long-input"
              placeholder="नाम / थर"
            />
          ...
          <p>
            वडा नं.
            <input
              value={form.bodartha_ward}
              onChange={(e) => setField("bodartha_ward", e.target.value)}
              type="text"
                ...
                <tr key={i}>
                  ...
                  <td className="action-cell">
                    ...
                    {form.transfers1.length > 1 && (
                      <button type="button" onClick={() => removeTransfer1(i)}>-</button>
                    )}
                  </td>
                </tr>
              ))}
                ...
                <tr key={i}>
                  ...
                  <td className="action-cell">
                    ...
                    {form.transfers2.length > 1 && (
                      <button type="button" onClick={() => removeTransfer2(i)}>-</button>
                    )}
                  </td>
                </tr>
              ))}
