...
const AnnualIncomeCertificate = () => {
  ...
  const addIncomeRow = () => {
    setIncomes(prev => [...prev, { id: prev.length + 1, ownerTitle: "Mr.", ownerName: "", relation: "", sourceDescription: "", sourceEntity: "", annualIncome: "", remark: "" }]);
  };

  const validate = () => {
    const requiredMain = ["totalIncomeNRs", "totalIncomeWords", "usdRate", "equivalentUSD", "equivalentUSDWords", "designation", "applicantName", "applicantAddress", "applicantCitizenship", "applicantPhone"];
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch("/api/forms/annual-income-certificate", {
        method: "POST",
      ...
      <form onSubmit={handleSubmit}>
        ...
        <p className="certificate-body">
          ...
          <select name="relation" value={formData.relation} onChange={handleChange}>
            <option>son</option><option>wife</option><option>daughter</option>
          </select>
          of
          ...
          of
          <select name="residencyType" value={formData.residencyType} onChange={handleChange}><option>permanent resident</option><option>temporary resident</option></select>
          of
          ...
          <input type="text" name="province" value={formData.province} onChange={handleChange} /> Nepal has following sources of income...
        </p>
            ...
            {incomes.map((income, idx) => (
              <tr key={income.id}>
                ...
                <td className="owner-name-cell">
                  <select name="ownerTitle" value={income.ownerTitle} onChange={e => handleIncomeChange(idx, e)}>
                    <option>Mr.</option><option>Mrs.</option><option>Ms.</option>
                  </select>
                  <input type="text" name="ownerName" value={income.ownerName} onChange={e => handleIncomeChange(idx, e)} required />
                </td>
                <td><input type="text" name="relation" value={income.relation} onChange={e => handleIncomeChange(idx, e)} required /></td>
                <td className="source-cell">
                  <input type="text" name="sourceDescription" value={income.sourceDescription} onChange={e => handleIncomeChange(idx, e)} required placeholder="Description" />
                  <label style={{display:'block',fontSize:'0.8em'}}>Income from</label>
                  <input type="text" name="sourceEntity" value={income.sourceEntity} onChange={e => handleIncomeChange(idx, e)} required placeholder="Entity" />
                </td>
                <td><input type="number" name="annualIncome" value={income.annualIncome} onChange={e => handleIncomeChange(idx, e)} required /></td>
                <td><input type="text" name="remark" value={income.remark} onChange={e => handleIncomeChange(idx, e)} /></td>
                <td>{idx === incomes.length - 1 && <button type="button" onClick={addIncomeRow} className="add-btn">+</button>}</td>
              </tr>
            ))}
          </tbody>
        </table>
        ...
        <div className="summary-section">
          <div className="summary-row">
            ...
            <input type="number" name="totalIncomeNRs" value={formData.totalIncomeNRs} onChange={handleChange} required />
          ...
          <div className="summary-row">
            ...
            <input type="number" step="0.01" name="usdRate" value={formData.usdRate} onChange={handleChange} required />
          </div>

          <div className="summary-row">
            <label>Equivalent to USD *</label>
            <input type="number" name="equivalentUSD" value={formData.equivalentUSD} onChange={handleChange} required />
            <label>(In words: *</label>
