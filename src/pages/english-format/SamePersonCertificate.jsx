...
const SamePersonCertificate = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await fetch("/api/forms/same-person-certificate", {
        method: "POST",
    ...
    <div className="same-person-container">
      <form onSubmit={handleSubmit}>
        ...
        <div className="subject-line">
          <strong>Subject: <u>Same Person Certificate</u></strong><br/>
        ...
        <p className="certificate-body">
          ...
          is
          <select name="doc1NameTitle" value={formData.doc1NameTitle} onChange={handleChange}><option>Mr.</option></select>
          ...
          is
          <select name="doc2NameTitle" value={formData.doc2NameTitle} onChange={handleChange}><option>Mr.</option></select>
          <input type="text" name="doc2Name" placeholder="Name 2" value={formData.doc2Name} onChange={handleChange} required />
          ( <select name="doc2Relation" value={formData.doc2Relation} onChange={handleChange}><option>Son</option></select>
          of
          <select name="doc2GuardianTitle" value={formData.doc2GuardianTitle} onChange={handleChange}><option>Mr.</option></select>
        ...
        <p className="certificate-body">
          ...
          is
          <select name="doc4NameTitle" value={formData.doc4NameTitle} onChange={handleChange}><option>Mr.</option></select>
          <input type="text" name="doc4Name" placeholder="Name 3" value={formData.doc4Name} onChange={handleChange} required />
          ( <select name="doc4Relation" value={formData.doc4Relation} onChange={handleChange}><option>Son</option></select>
          of
          <select name="doc4GuardianTitle" value={formData.doc4GuardianTitle} onChange={handleChange}><option>Mr.</option></select>
          <input type="text" name="doc4GuardianName" placeholder="Name 3's Father" value={formData.doc4GuardianName} onChange={handleChange} required /> ) both are the same person.
        </p>

        <p className="certificate-body summary-line">
          So, this is certified that the name
          <select name="finalName1Title" value={formData.finalName1Title} onChange={handleChange}><option>Mr.</option></select>
          <input type="text" name="finalName1" placeholder="Name 1" value={formData.finalName1} onChange={handleChange} required />
          and
          <select name="finalName2Title" value={formData.finalName2Title} onChange={handleChange}><option>Mr.</option></select>
