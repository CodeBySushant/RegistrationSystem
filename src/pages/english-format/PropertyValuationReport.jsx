...
const PropertyValuationReport = () => {
  ...
  const validate = () => {
    const requiredMain = [
      "applicantNameBody", "guardianName", "wardNo2",
      "valuationNRs", "usdRate", "equivalentUSD",
    ...
    if (!okProp) return { ok: false, missing: "property (at least one with description & ownerName)" };
    return { ok: true };
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch("/api/forms/property-valuation-report", {
        method: "POST",
    ...
    <div className="property-valuation-container">
      <form onSubmit={handleSubmit}>
        ...
        <div className="subject-line">
          <strong>Subject: <u>Property Valuation Report</u></strong><br/>
        ...
        <p className="certificate-body">
          ...
          <select name="relation" value={formData.relation} onChange={handleChange}><option>Grandson</option><option>Son</option><option>Daughter</option></select>
          of
          ...
          District
          <input type="text" name="propertyDistrict" value={formData.propertyDistrict} onChange={handleChange} />, Nepal which have been valuated as per the current prevailing market rate as follows:
        </p>
        ...
        <table className="properties-table">
          <thead>
            <tr>
              <th>SN</th><th>Description</th><th>Owner's Name</th><th>Location</th><th>Plot No.</th><th>Area in Ropani ..</th><th></th>
            </tr>
          </thead>
          <tbody>
            {properties.map((prop, idx) => (
              <tr key={prop.id}>
                <td>{idx + 1}</td>
                <td><input type="text" name="description" value={prop.description} onChange={(e) => handlePropertyChange(idx, e)} required /></td>
                <td className="owner-name-cell">
                  <select name="ownerTitle" value={prop.ownerTitle} onChange={(e) => handlePropertyChange(idx, e)}>
                    <option>Mr.</option><option>Mrs.</option><option>Ms.</option>
                  </select>
                  <input type="text" name="ownerName" value={prop.ownerName} onChange={(e) => handlePropertyChange(idx, e)} required />
                </td>
                <td><input type="text" name="location" value={prop.location} onChange={(e) => handlePropertyChange(idx, e)} required /></td>
                <td><input type="text" name="plotNo" value={prop.plotNo} onChange={(e) => handlePropertyChange(idx, e)} required /></td>
                <td><input type="text" name="area" value={prop.area} onChange={(e) => handlePropertyChange(idx, e)} required /></td>
                <td>{idx === properties.length - 1 && <button type="button" onClick={addPropertyRow} className="add-btn">+</button>}</td>
              </tr>
            ))}
          </tbody>
        </table>
