...
const PowerOfAttorney = () => {
  ...
  const addRelative = () => {
    setRelatives(prev => [...prev, { id: prev.length + 1, title: "Mr.", name: "", address: "", age: "", signature: "", relationship: "" }]);
  };

  const validate = () => {
    const requiredMain = [
      "deceasedName", "passportNo", "expiredOn", "dueTo", "issuedPowerOfAttorneyOn",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch("/api/forms/power-of-attorney", {
        method: "POST",
    ...
    <div className="power-of-attorney-container">
      <form onSubmit={handleSubmit}>
        ...
        <div className="subject-line">
          <strong>Subject: <u>POWER OF ATTORNEY</u></strong><br />
        ...
        <p className="certificate-body">
          ...
          <br />
          We have issued this power of attorney in favor of the Nepalese Ambassador/Chargé d’Affaires a.i. at the Nepalese Embassy to act on our behalf before judicial,
          labour and other concerned government authorities.
          <br />
          The Nepalese Ambassador/Chargé d’Affaires a.i. is hereby authorized to defend, receive dues and any amount as decided by the judicial/administrative
          authorities, settle issues by compromise, pay lawyer's fees from compensation amount received, follow up all the matters and take necessary actions.
          The Nepalese Ambassador may delegate these powers to subordinates at the Embassy or to the designated lawyer in
          ...
          <input type="date" name="issuedPowerOfAttorneyOn" value={formData.issuedPowerOfAttorneyOn} onChange={handleChange} required /> A.D. from Nepal.
        </p>

        <div className="undersigned"><strong><u>Undersigned</u></strong></div>

        <table className="relatives-table">
          <thead>
            <tr>
              <th>SN</th><th>Title</th><th>Relative's Name</th><th>Address</th><th>Age</th><th>Signature/Thumb</th><th>Relationship</th><th></th>
            </tr>
          </thead>
          <tbody>
            {relatives.map((r, idx) => (
              <tr key={r.id}>
                <td>{idx + 1}</td>
                <td>
                  <select name="title" value={r.title} onChange={(e) => handleRelativeChange(idx, e)}>
                    <option>Mr.</option><option>Mrs.</option><option>Ms.</option>
                  </select>
                </td>
                <td><input type="text" name="name" value={r.name} onChange={(e) => handleRelativeChange(idx, e)} required /></td>
                <td><input type="text" name="address" value={r.address} onChange={(e) => handleRelativeChange(idx, e)} required /></td>
                <td><input type="number" name="age" value={r.age} onChange={(e) => handleRelativeChange(idx, e)} min="0" /></td>
                <td><input type="text" name="signature" value={r.signature} onChange={(e) => handleRelativeChange(idx, e)} placeholder="Sign/Thumb" /></td>
                <td>
                  <select name="relationship" value={r.relationship} onChange={(e) => handleRelativeChange(idx, e)}>
                    <option>Grandfather</option><option>Grandmother</option><option>Father</option><option>Mother</option><option>Son</option><option>Daughter</option><option>Spouse</option>
                  </select>
                </td>
                <td>{idx === relatives.length - 1 && <button type="button" onClick={addRelative} className="add-btn">+</button>}</td>
              </tr>
            ))}
          </tbody>
        </table>
